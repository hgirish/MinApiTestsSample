...
public class TodoEndpointsV1Tests : IClassFixture<TestWebApplicationFactory<Program>>
{
    ...
    public async Task PostTodosWithValidationProblems(TodoDto todo, string errorMessage)
    {
        ...
        Assert.Collection(problemResult.Errors, 
            (error) => Assert.Equal(errorMessage, error.Value.First()));
    }
