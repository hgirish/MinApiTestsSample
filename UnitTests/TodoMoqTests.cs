...
public class TodoMoqTests
{
    ...
    public async Task CreateTodoCreatesTodoInDatabase()
    {
        ...
        var mock = new Mock<ITodoService>(MockBehavior.Strict);
        mock.Setup(m => m.Add(It.Is<Todo>(t => 
        t.Title == newTodo.Title &&
        t.Description == newTodo.Description &&
        t.IsDone == newTodo.IsDone)))
